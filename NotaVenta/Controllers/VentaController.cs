...
namespace NotaVenta.Controllers
{
    public class VentaController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult ObtenerDocumentosSaldados(string RutAuxiliar, string CodAux)
        {
            double MontoTotal = 0;
            double Saldo = 0;
            try
            {
                List<SaldosModel> Saldos = new List<SaldosModel>();
                Saldos = controlDisofi().ObtenerDocumentosSaldados(RutAuxiliar, CodAux, baseDatosUsuario(), 5);
